$(document).ready(function() {
    ...
    $(document).bind('search', function(event, data) {
        var url = "https://elastic.whit-e.com/wiki/wiki/_search?pretty";
                ...
                for(var i = 0; i < data.hits.hits.length; i++) {
			    	...
			    	Object.keys(data.hits.hits[i]._source).forEach(function(key) {
						...
						if(String(dynamicResponse.values[j]).startsWith('http')) {
							...
							dynamicResponse.values[j] = "<pre style='white-space: pre-wrap;'><code class='html htmlContent'>" + dynamicResponse.values[j].substring(0, 500).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + "</code></pre>"
						} else if(dynamicResponse.values[j].length > 500) {
							dynamicResponse.values[j] = dynamicResponse.values[j].substring(0, 500);
						}
						j++;
					});
                	var j = 0;
